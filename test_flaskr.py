...
class BookTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.new_book = {"title": "Understanding Peoples Personalities", "author": "Milton Jesumbo A.", "rating": 5}
    ...
    def test_get_book_search_with_results(self):
    	# 'Still'  it has to be in the database. 
        res = self.client().post('/books', json={'search': 'Still'})
        data = json.loads(res.data)

        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        #  note that here, it is used  assertTrue()
        self.assertTrue(data['total_books'])
        #  here 1，  it has to be consistent with the database data. 
